...
in {
  options = {
    herbstluftwm.enable = mkEnableOption "Enable hlwm";
    herbstluftwm.wallpaper = mkOption {
      type = types.str;
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    xsession.windowManager.herbstluftwm = {
      ...
      feh --bg-fill "${config.home.homeDirectory}/.config/herbstluftwm/wallpaper"
      eww kill
      eww open "$(hostname)"
    '';
    };
    ...
    home.file = {   
      "hlwm_theme" = {
        source = config.themes.base16.herbstluftwm;
        target = ".config/herbstluftwm/theme.sh";
        executable = true;
      };
      "hlwm_wallpaper" = {
        source = ../wallpapers/${cfg.wallpaper};
